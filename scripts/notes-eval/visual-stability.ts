 ...
 * Reports how often the outcome (teachingPoint → kind @ placement) agrees.
 *
...
async function plan(sheet: Sheet, fallbackTopic: string): Promise<string> {
  ...
  if (!res.ok) return `planner_error_${res.status}`;
  const r = await res.json();
  return r.visual ? `${r.teachingPoint} → ${r.visual.kind} @ ${r.visual.placement}` : `${r.teachingPoint} → none (${r.reason})`;
}
...
for (const topic of TOPICS) {
  ...
  for (const writer of WRITERS) {
    for (let i = 0; i < RUNS; i++) {
      ...
      fixedSheet ??= sheet;
      const outcome = await plan(sheet, topic);
      outcomes.push(outcome);
      console.log(`  ${writer} #${i + 1}: ${outcome}`);
    }
  }
  console.log(`  sheets:  ${agreement(outcomes)}`);
  if (new Set(outcomes).size === 1) sheetAgree++;

  if (fixedSheet) {
    const replans: string[] = [];
    for (let i = 0; i < RUNS; i++) replans.push(await plan(fixedSheet, topic));
