...
Terse vocabulary + architecture reference. Attached to `/grill-with-docs` and `/to-prd`. Describes `main` as it actually is. Not a tutorial. (No `CLAUDE.md` exists in the repo; this file is the map.)
...
- React Query keys, array form. QBank keys to invalidate on mutation: `["qbank-count"]`, `["qbank-domains"]`/`["qbank-domain-meta"]`, `["qbank-meta"]`, `["qbank-sessions"]` (note the actual key is `qbank-domain-meta`, not `qbank-domains`). Profile keys: `["profile",userId]`, `["profile-premium",userId]`, `["model-preference",userId]`.
...
- **QBank types centralized** — the QBank client types (`Question`, `QuestionMedia`, `OptionKey`, `SessionAnswer`, `SessionState`) live in `src/lib/qbank-types.ts`, imported by QBankContext / QBankSession / QBankSummary. The legacy `use-qbank.ts` hook file was deleted (it held the same types).
